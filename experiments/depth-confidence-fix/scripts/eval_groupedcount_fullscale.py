...
GROUPEDCOUNT_CKPT = GROUPEDCOUNT_CKPTS[-1]  # latest epoch
LABEL = "groupedcount-fullscale"

results = {}
for (ds, ind, depth), v in ec.get_baseline_results().items():
    results[("baseline", ds, ind, depth)] = v

print(f"loading {LABEL}: {GROUPEDCOUNT_CKPT}", flush=True)
model = GRITSCRFDiploidIndel.load_from_checkpoint(
    GROUPEDCOUNT_CKPT, map_location=ec.device, strict=False).eval().to(ec.device)
