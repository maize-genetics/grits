"""
Full-scale read-count checkpoint evaluation: ALL real IDX-INBRED/IDX-HYB/
IDX-RIL2 samples (5 each, 15 total), all 5 depths, founder pair_acc AND
ibd_adjusted_accuracy, v3-K25 baseline vs the full-scale-trained
readcount checkpoint (--emit-read-counts genuinely non-null for both,
via the *_wcount.npy real-data conversions). Reports everything, not
just the Oh43xIl14H pair used for the validation-scale round -- per
user request, comprehensive coverage even where a sample looks worse.
"""
import sys
...
CKPTS = {
    ...
    "readcount-fullscale": ("/workdir/zrm22/HackathonJun2026/grits_workdir/indel_baseline/checkpoints/"
                            "diploid-indel-v3-readcount-fullscale/d-epoch=03-val_pair_acc=0.4533.ckpt"),
}
K = 25
...
for label, ckpt_path in CKPTS.items():
    ...
    for ds, ind in all_samples:
        for depth in DEPTHS:
            ...
            print(f"{label:<20}{ds:<12}{ind:<12}{depth:<7}pair_acc={pa*100:6.2f}%  ibd_adj={ia*100:6.2f}%")
    print()

print("\n=== SUMMARY: mean pair_acc / ibd_adj by kind and depth, baseline vs readcount-fullscale ===")
